...
void read_db_torrents_sql()
{
	...
	try
	{
		...
		{
			Csql_result result = Csql_query(m_database, "select info_hash, @fid, download_multiplier, upload_multiplier from @files where flags & 2").execute();
			while (Csql_row row = result.fetch_row())
			{
		...
		if (m_config.m_auto_register && !m_torrents.empty())
			return;
		Csql_result result = Csql_query(m_database, "select info_hash, @completed, @fid, ctime, download_multiplier, upload_multiplier from @files where @fid >= ?")(m_fid_end).execute();
...
void read_db_users()
{
	...
	try
	{
		Csql_query q(m_database, "select @uid, download_multiplier, upload_multiplier");
...
{
	if (m_use_sql && m_config.m_log_announce && v.m_left == 0)
	{
	...
	if (!m_config.m_anonymous_announce || file.download_multiplier>0 && !user)
		return bts_unregistered_torrent_pass;
